  ...
  toDto(
    ...
    return {
      ...
      drops: drops.map((drop: any) => {
        const product = products.filter((product: ProductsEntity) => product.drop_id === drop.id);

        const productPrice = productsPrice.filter((pp) => product.some((p) => p.id === pp.id));
        return {
          id: drop.id,
          name: drop.name,
          ...
          price: productPrice.reduce((acc, productPrice) => acc + productPrice.price, 0),
      }}),
    };
  }
}
